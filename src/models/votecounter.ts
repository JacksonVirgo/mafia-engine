...
export async function deletePlayerAndVotes(
  votecountId: number,
  discordId: string
) {
  try {
...
export type FullVoteCount = NonNullable<
  Awaited<ReturnType<typeof getVoteCounter>>
>;
...
export async function getVoteCounterPlayerOrThrow(
  vcId: number,
  discordId: string
) {
