...
import SetVoteWeightModal from "../../modals/setExtraVotes";

export default new CustomUserSelectMenuBuilder(
  "manage-vc-select-players-vote-extra"
)
  .onGenerate((builder) =>
    builder
      .setMaxValues(1)
      .setMinValues(1)
      .setPlaceholder("The player to change base votes on them")
  )
  .onExecute(async (i) => {
    if (!i.guild)
    ...
    if (!player)
      throw new InteractionError(
        "The player must be registered in this vote-count"
      );
