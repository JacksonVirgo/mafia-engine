...
import {
  genCreateVoteCountEmbed,
  genStateEmbed,
} from "../../views/votecounter";

export default new Modal("manage-vc-players-set-vote-extra")
  .set((modal) => {
    modal.setTitle("Set Extra Votes");
    const row = new ActionRowBuilder<ModalActionRowComponentBuilder>();
    row.addComponents(
      new TextInputBuilder()
        .setCustomId("vote-weight")
        .setLabel("Vote Amount")
  ...
  .onExecute(async (i: ModalSubmitInteraction<CacheType>, cache) => {
    ...
    if (!voteWeightRaw)
      throw new InteractionError("No vote weight was provided");

    const voteWeight = Number(voteWeightRaw);
    if (isNaN(voteWeight))
      throw new InteractionError("The provided vote weight is not a number");
    ...
    await prisma.player.update({
      ...
      data: {
        additionalVotes: voteWeight,
      },
    });

    const newVC = await getVoteCounter({ channelId: i.channel.id });
    if (!newVC)
      return i.reply({ ...genCreateVoteCountEmbed(), ephemeral: true });
